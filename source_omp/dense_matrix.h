...
class dense_matrix{
    ...
    private:
    void insert_cars(sparse_matrix &SM, char color_car, const unsigned int nthreads);
...
dense_matrix::dense_matrix(unsigned int row, unsigned int col, sparse_matrix &red, sparse_matrix &blu):row(row),col(col){
    ...
    unsigned int nprocs=omp_get_num_procs();
    //paralelizzando queste due sezioni era più lento che dedicando in sequenza i 4 threads a ciascuno
    insert_cars(blu,color_blu,nprocs);
    insert_cars(red,color_red,nprocs);
...
void dense_matrix::insert_cars(sparse_matrix &SM, char color_car,const unsigned int nthreads){
    unsigned int coord;
    const char blu='1';
    const char red='2';
